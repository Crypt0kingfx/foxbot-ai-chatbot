...
/* Studio Action Feedback Patch
   Adds a visible result panel so button clicks clearly show what happened.
*/
(function () {
  if (window.foxbotActionFeedbackInstalled) return;
  window.foxbotActionFeedbackInstalled = true;

  function ensureActionPanel() {
    let panel = document.getElementById("foxbotActionPanel");

    if (!panel) {
      panel = document.createElement("div");
      panel.id = "foxbotActionPanel";
      panel.style.position = "fixed";
      panel.style.right = "22px";
      panel.style.bottom = "22px";
      panel.style.width = "min(420px, calc(100vw - 44px))";
      panel.style.maxHeight = "320px";
      panel.style.overflow = "auto";
      panel.style.zIndex = "99999";
      panel.style.border = "1px solid rgba(255, 170, 70, .45)";
      panel.style.borderRadius = "18px";
      panel.style.background = "rgba(8, 10, 8, .96)";
      ...
      panel.innerHTML = `
        <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:8px;">
          <strong style="color:#ffb15a;">FoxBot Action Result</strong>
          <button id="foxbotActionPanelClose" style="border:0;border-radius:10px;padding:5px 9px;cursor:pointer;">Close</button>
        </div>
        <pre id="foxbotActionPanelBody" style="white-space:pre-wrap;margin:0;font-size:12px;line-height:1.45;color:#e9fbe9;"></pre>
      `;

      document.body.appendChild(panel);

      const closeBtn = document.getElementById("foxbotActionPanelClose");
      if (closeBtn) {
  ...
  function showActionResult(title, data) {
    const panel = ensureActionPanel();
    const body = document.getElementById("foxbotActionPanelBody");
    ...
    clearTimeout(window.foxbotActionPanelTimer);
    window.foxbotActionPanelTimer = setTimeout(() => {
      const current = document.getElementById("foxbotActionPanel");
      if (current) current.style.display = "none";
    }, 12000);
  }

  window.showFoxBotActionResult = showActionResult;

  const oldStudioResult = window.foxbotStudioResult;
  window.foxbotStudioResult = function (title, data) {
    showActionResult(title, data);
    if (typeof oldStudioResult === "function") {
      return oldStudioResult(title, data);
    }
  };

  const oldVisibleOutput = window.foxbotVisibleOutput;
  window.foxbotVisibleOutput = function (title, data) {
    showActionResult(title, data);
    if (typeof oldVisibleOutput === "function") {
      return oldVisibleOutput(title, data);
    }
  };
  ...
  function wrapActionFunction(name) {
    ...
    window[name] = async function (...args) {
      showActionResult(`${name} running...`, args.join(" ") || "Working...");

      try {
        const result = await original.apply(this, args);

        if (result !== undefined) {
          showActionResult(`${name} complete`, result);
        }

        return result;
      } catch (error) {
        showActionResult(`${name} failed`, String(error));
        throw error;
      }
    };
  }
...
/* Admin Event Bridge Test Panel v1 */
(function () {
  if (window.foxbotEventBridgePanelInstalled) return;
  window.foxbotEventBridgePanelInstalled = true;

  function bridgePanelResult(title, data) {
    if (typeof window.showFoxBotActionResult === "function") {
      window.showFoxBotActionResult(title, data);
    }

    const out = document.getElementById("eventBridgePanelOutput");
    if (out) {
      out.textContent = typeof data === "string" ? data : JSON.stringify(data, null, 2);
    }
  }

  async function postBridgeEvent(eventType, username, amount, eventId) {
    const body = {
    ...
    bridgePanelResult("Event Bridge running...", body);

    const res = await fetch("/api/blaze/event-bridge", {
      method: "POST",
      headers: {
    ...
    const data = await res.json();
    bridgePanelResult("Event Bridge Result", data);
    return data;
  }

  async function viewRecognitionLog() {
    bridgePanelResult("Loading Recognition Log...", "Working...");

    const res = await fetch("/recognition");
    const data = await res.json();

    bridgePanelResult("Recognition Log", data);
    return data;
  }

  function createEventBridgePanel() {
    if (document.getElementById("eventBridgeTestPanel")) return;

    const panel = document.createElement("section");
    panel.id = "eventBridgeTestPanel";
    panel.className = "section active";
    panel.style.margin = "18px 0";
    panel.style.padding = "18px";
    panel.style.border = "1px solid rgba(255, 170, 70, .35)";
    panel.style.borderRadius = "18px";
    panel.style.background = "rgba(255,255,255,.035)";

    panel.innerHTML = `
      <div class="card">
        <h2>Blaze Event Bridge Test Panel</h2>
        <p>Trigger fake Blaze events from the admin page. These test recognition, FoxCoins, and logging without PowerShell.</p>

        <div class="row" style="display:flex;flex-wrap:wrap;gap:10px;margin:12px 0;">
          <input id="eventBridgeUsername" value="BridgeFan" placeholder="Username" style="min-width:180px;">
          <input id="eventBridgeAmount" value="1" placeholder="Amount" style="width:110px;">
        </div>

        <div class="row" style="display:flex;flex-wrap:wrap;gap:10px;margin:12px 0;">
          <button class="action" onclick="foxbotPostBridgeEvent('new_follow')">Test Follow</button>
          <button class="action" onclick="foxbotPostBridgeEvent('user_voted', 10)">Test Vote x10</button>
          <button class="action" onclick="foxbotPostBridgeEvent('new_sub')">Test Sub</button>
          <button class="action" onclick="foxbotPostBridgeEvent('user_tipped', 5)">Test Tip $5</button>
          <button class="action" onclick="foxbotPostBridgeEvent('incoming_raid')">Test Raid</button>
          <button class="secondary" onclick="foxbotPostBridgeDuplicate()">Test Duplicate</button>
          <button class="secondary" onclick="foxbotViewRecognitionLog()">View Recognition Log</button>
        </div>

        <pre id="eventBridgePanelOutput" class="out" style="white-space:pre-wrap;max-height:300px;overflow:auto;">Ready.</pre>
      </div>
    `;
  ...
  window.foxbotPostBridgeEvent = function (eventType, forcedAmount) {
    const username = document.getElementById("eventBridgeUsername")?.value || "BridgeFan";
    const rawAmount = document.getElementById("eventBridgeAmount")?.value || 1;
    const amount = forcedAmount || rawAmount;

    return postBridgeEvent(eventType, username, amount);
  };

  window.foxbotPostBridgeDuplicate = async function () {
    const username = document.getElementById("eventBridgeUsername")?.value || "DuplicateFan";
    const eventId = "admin-dupe-test-001";

    await postBridgeEvent("new_follow", username, 1, eventId);
    return postBridgeEvent("new_follow", username, 1, eventId);
  };

  window.foxbotViewRecognitionLog = viewRecognitionLog;

  document.addEventListener("DOMContentLoaded", createEventBridgePanel);
})();
