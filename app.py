...
async def blaze_service_event(raw_event: dict):
    ...
    if not mapped_event:
        ...
        studio_log(f"⚠️ Unmapped Blaze service event: {event_name}")
...
# === FoxBot Giveaway Admin Control v1 ===
@app.get("/admin-giveaways")
def foxbot_admin_giveaways_page_v1():
    from fastapi.responses import HTMLResponse
...
<html>
<head>
    <title>FoxBot Giveaway Control</title>
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <style>
        body {
            margin: 0;
        ...
        .hero {
            background: linear-gradient(135deg, rgba(255,122,24,.22), rgba(132,204,22,.08));
            border: 1px solid rgba(255,255,255,.12);
            border-radius: 22px;
            padding: 24px;
            margin-bottom: 18px;
...
<div class="wrap">
    <div class="hero">
        <h1>FoxBot Giveaway Control</h1>
        <div class="sub">Start giveaways, add/test entries, view entries, pick winners, and clear the queue.</div>
        <div style="margin-top:10px;"><a href="/admin">Back to Admin Hub</a></div>
    </div>

    <div class="grid">
        <div class="card">
            <div class="sub">Total Entries</div>
            <div id="entryCount" class="big">0</div>
        </div>
        <div class="card">
            <div class="sub">Status</div>
            <div id="statusText" class="big">Ready</div>
        </div>
        <div class="card">
            <div class="sub">Last Action</div>
            <div id="lastAction" class="big">None</div>
        </div>
    </div>

    <div class="card">
        <h2>Giveaway Actions</h2>
        <button onclick="runGiveawayCommand('!giveaway')">Announce Giveaway</button>
        <button class="secondary" onclick="runGiveawayCommand('!entries')">Show Entries</button>
        <button onclick="runGiveawayCommand('!pickwinner')">Pick Winner</button>
        <button class="danger" onclick="clearEntries()">Clear Entries</button>
    </div>

    <div class="card">
        <h2>Add/Test Entry</h2>
        <input id="entryUser" placeholder="viewer name, example: crypt0k1ng96">
...
async function refreshStatus() {
    try {
        const data = await api("/api/giveaways/status");
        document.getElementById("entryCount").textContent = data.count || 0;
        document.getElementById("statusText").textContent = data.count > 0 ? "Active" : "Ready";
...
async function runGiveawayCommand(command) {
    document.getElementById("lastAction").textContent = command;
    const data = await api("/api/giveaways/command", {
        method: "POST",
        headers: {"Content-Type": "application/json"},
        body: JSON.stringify({command, username: "crypt0k1ng96"})
    });
    show(data);
    refreshStatus();
}
...
async function addEntry() {
    const username = document.getElementById("entryUser").value.trim() || "testviewer";
    document.getElementById("lastAction").textContent = "Add " + username;
    const data = await api("/api/giveaways/command", {
        method: "POST",
        headers: {"Content-Type": "application/json"},
        body: JSON.stringify({command: "!enter", username})
    });
    show(data);
    refreshStatus();
}
...
async function clearEntries() {
    document.getElementById("lastAction").textContent = "Clear";
    const data = await api("/api/giveaways/clear", {method: "POST"});
    show(data);
    refreshStatus();
}
...
@app.get("/api/giveaways/status")
def foxbot_giveaways_status_v1():
    entries = globals().get("giveaway_entries", [])
    if entries is None:
        entries = []
    ...
    return {
        "ok": True,
        "count": len(entries),
        "entries": list(entries)[-250:],
        "commands": ["!giveaway", "!enter", "!entries", "!pickwinner"]
    }


@app.post("/api/giveaways/command")
async def foxbot_giveaways_command_v1(payload: dict):
    command = str(payload.get("command", "")).strip()
    username = str(payload.get("username", "crypt0k1ng96")).strip() or "crypt0k1ng96"

    if command not in ["!giveaway", "!enter", "!entries", "!pickwinner"]:
        return {
            "ok": False,
            "error": "Unsupported giveaway command.",
            "allowed": ["!giveaway", "!enter", "!entries", "!pickwinner"]
        }

    try:
        result = chat(message=command, username=username, admin=True)
    except TypeError:
        try:
            result = chat(command, username, True)
        except TypeError:
            result = chat(message=command, username=username)
...
@app.post("/api/giveaways/clear")
async def foxbot_giveaways_clear_v1():
    entries = globals().setdefault("giveaway_entries", [])
    ...
    return {
        "ok": True,
        "message": "Giveaway entries cleared.",
        "count": len(globals().get("giveaway_entries", []))
    }
